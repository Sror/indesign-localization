...
var InstalledFonts = '<?php
	...
	while($row = mysql_fetch_assoc($result))
		printf('<option value="%d">[%s] %s</option>',
			...
			htmlspecialchars('('.$row['family'].') '. $row['name'],ENT_QUOTES));
?>';
			...
			<div class="toolbar">
				...
				<div class="options">
					...
					<div class="optionOff" onmouseover="this.className='optionOn'" onmouseout="this.className='optionOff'" title="<?php echo $lang->display('Close'); ?>">
						<a href="javascript:void(0);" onclick="window.location = '<?php echo $back; ?>'">
					...
					<div class="search">
						<table id="navlay">
							...
							<tr>
								...
								echo '</td><td>';
								if($companyID){
									$result = mysql_query("SELECT campaigns.campaignID, campaigns.campaignName FROM campaigns LEFT JOIN brands ON brands.brandID = campaigns.brandID WHERE campaigns.campaignStatus='1' AND brands.companyID = ${companyID}");
									echo '
								...
								if($campaignID){
									$result = mysql_query("SELECT artworks.artworkID, artworks.artworkName FROM artworks WHERE artworks.live = 1 AND artworks.campaignID = ${campaignID}");
									echo '
