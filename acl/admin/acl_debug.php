...
switch ($_GET['action']) {
    case 'Submit':
		...
		//$result = $acl_api->acl_query('system', 'email_pw', 'users', '1', NULL, NULL, NULL, NULL, TRUE);
		$result = $acl_api->acl_query(	$_GET['aco_section_value'],
										$_GET['aco_value'],
		...
		$result['query'] = str_replace(	'a.id,a.allow,a.return_value',
										'	a.id,
											...
											$result['query']);
		$rs = $acl_api->db->Execute($result['query']);
...
$smarty->display('paglacl/acl_debug.tpl');
?>