...
switch(strtolower(trim($group_type))) {
	case 'axo':
		$group_type = 'axo';
		$table = $acl_api->_db_table_prefix . 'axo';
		$group_table = $acl_api->_db_table_prefix . 'axo_groups';
		$group_sections_table = $acl_api->_db_table_prefix . 'axo_sections';
		$group_map_table = $acl_api->_db_table_prefix . 'groups_axo_map';
		$object_type = 'Access eXtension Object';
		break;
	default:
		$group_type = 'aro';
		$table = $acl_api->_db_table_prefix . 'aro';
		$group_table = $acl_api->_db_table_prefix . 'aro_groups';
		$group_sections_table = $acl_api->_db_table_prefix . 'aro_sections';
		$group_map_table = $acl_api->_db_table_prefix . 'groups_aro_map';
...
switch ($_POST['action']) {
	case 'Remove':
		...
		while (list($object_section_value,$object_array) = @each($selected_object_array)) {
			$acl_api->debug_text('Assign: Object ID: '. $object_section_value .' to Group: '. $_POST['group_id']);
			
			foreach ($object_array as $object_value) {
				$acl_api->del_group_object($_POST['group_id'], $object_section_value, $object_value, $group_type);
			}
		}
		
		//Return page.
		$acl_api->return_page($_SERVER['PHP_SELF'] .'?group_type='. $_POST['group_type'] .'&group_id='. $_POST['group_id']);
	...
	case 'Submit':
		...
		while (list($object_section_value,$object_array) = @each($selected_object_array)) {
			...
			foreach ($object_array as $object_value) {
				$acl_api->add_group_object($_POST['group_id'], $object_section_value, $object_value, $group_type);
			}
		}
	...
	//Get group name.
	$group_data = $acl_api->get_group_data($_GET['group_id'], $group_type);
...
$smarty->display('paglacl/assign_group.tpl');
?>
