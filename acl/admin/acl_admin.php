...
switch ($_POST['action']) {
	...
	case 'Submit':
		...
		if (!empty($_POST['acl_id'])) {
			//Update existing ACL
			$acl_id = $_POST['acl_id'];
			if ($acl_api->edit_acl($acl_id, $selected_aco_array, $selected_aro_array, $_POST['aro_groups'], $selected_axo_array, $_POST['axo_groups'], $_POST['allow'], $enabled, $_POST['return_value'], $_POST['note'], $_POST['acl_section']) == FALSE) {
				echo 'ERROR editing ACL, possible conflict or error found...<br />' . "\n";
				exit;
			}
		} else {
			//Insert new ACL.
			if ($acl_api->add_acl($selected_aco_array, $selected_aro_array, $_POST['aro_groups'], $selected_axo_array, $_POST['axo_groups'], $_POST['allow'], $enabled, $_POST['return_value'], $_POST['note'], $_POST['acl_section']) == FALSE) {
	...
	default:
		...
		if ($_GET['action'] == 'edit' AND !empty($_GET['acl_id'])) {
			...
			foreach (array('aco','aro','axo') as $type) {
				...
				$query = '
					...
					INNER JOIN '. $acl_api->_db_table_prefix . $type .' b ON b.section_value=a.section_value AND b.value=a.value
					INNER JOIN '. $acl_api->_db_table_prefix . $type .'_sections c ON c.value=a.section_value
				...
				if (is_object($rs)) {
					while ($row = $rs->FetchRow()) {
						...
						$acl_api->debug_text("Section Value: $section_value Value: $value Section: $section ACO: $aco");
			...
			foreach (array('aro','axo') as $type) {
				...
				$query = '
					SELECT group_id
					FROM '. $acl_api->_db_table_prefix . $type .'_groups_map
			...
			$acl_api->debug_text('NOT EDITING ACL');
			$allow=1;
			$enabled=1;
		...
		foreach (array('aco','aro','axo') as $type) {
			...
			$query = '
				...
				FROM '. $acl_api->_db_table_prefix . $type .'
				WHERE hidden=0
				ORDER BY section_value,order_value,name';
			$rs = $db->SelectLimit($query,$acl_api->_max_select_box_items);
		...
		$smarty->assign('options_aro_groups', $acl_api->format_groups($acl_api->sort_groups('ARO')) );
		$smarty->assign('selected_aro_groups', $selected_aro_groups);
		
		//Grab formatted AXO Groups for select box
		$smarty->assign('options_axo_groups', $acl_api->format_groups($acl_api->sort_groups('AXO')) );
...
$smarty->display('paglacl/acl_admin.tpl');
?>