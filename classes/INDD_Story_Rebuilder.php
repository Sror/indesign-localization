...
class INDD_text_Rebuilder{
	...
	protected function removeDefineCellStyle() {
		$DefineCellStyleReg = '/^<DefineCellStyle(.*?[^\\\\](?:>)?)>\r\n/ui';
		//return $this->FindAndRemove($DefineCellStyleReg, $this->DefineCellStyle[]);
		while($this->FindAndRemoveWithOptions($DefineCellStyleReg, $this->DefineCellStyle, 'DefineCellStyle'));
	}
	...
	protected function rebuildhead(){
	    ...
	    foreach($this->DefineCellStyle as $DefineCellStyle){
		    if(empty($DefineCellStyle)) continue;
		    $str .= $DefineCellStyle;
	    }
