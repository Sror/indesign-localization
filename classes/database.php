...
class Database {
	...
	public function TenderTask($artworkID, $targetLangID, $translatorIDs, $proofreaderIDs, $deadline, $brief="", $attachment="", $trial=0, $start=true, $notes="",$storyGroup=0) {
		...
		if(mysql_num_rows($art_result)) {
			...
			foreach($translatorIDs as $translatorID=>$tdeadline) {
				...
				$update = sprintf("INSERT INTO tasks
                                        (desiredLanguageID, translatorID, tdeadline, attachment, artworkID, creatorID, lastUpdate,
                                        deadline, taskStatus, vCode, brief, serviceCurrencyID, serviceCharge, trial, notes, artwork_storygroup_id)
                                        VALUES
                                        (%d, %d, %s, '%s', %d, %d, NOW(), '%s', 7, '%s', '%s', %d, %f, %d, '%s',%d)",
                                        $targetLangID,
                                        $translatorID,
