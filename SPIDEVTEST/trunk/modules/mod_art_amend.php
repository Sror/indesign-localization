...
// hack to allow AJAX updating of MT_FLAG
if( isset($_REQUEST['mt_flag']) && isset($_REQUEST['p']) ) {
	$result = mysql_query("UPDATE paragraphs SET mt_flag = '".$_REQUEST['mt_flag']."' WHERE uID = '".$_REQUEST['p']."'") or die(mysql_error());
	...
	<?php
		...
		if($type == "TEXT") {
			...
			$query_para = sprintf("SELECT paralinks.uID as ParalinkID, paralinks.ParaID, paralinks.StoryGroup,
								...
								paragraphs.ParaText, paragraphs.mt_flag
								FROM paralinks
			...
			if(mysql_num_rows($result_para)) {
				...
				while($row_para = mysql_fetch_assoc($result_para)) {
					...
					echo '</p>';
					echo '<p><a href="javascript:void(0);" onclick="if(confirm(\'Are you sure you want to split this paragraph?\')) splitPara(\'amendForm\',\'para\','.$PL.');"><img src="'.IMG_PATH.'ico_split.png" title="'.$lang->display('Split').'" /></a>';
					
					// flag for multi-translation
					if($row_para['mt_flag']) {
					
						echo ' <a href="javascript:void(0);" onclick="AjaxPost(\'artworkID='.$artworkID.'&amp;p='.$row_para['ParaID'].'&amp;mt_flag=0\',\'\',\'modules/mod_art_amend.php\'); document.getElementById(\'mt_icon-'.$row_para['ParaID'].'\').src=\''.IMG_PATH.'mt_flag.jpg\';"><img id="mt_icon-'.$row_para['ParaID'].'" src="'.IMG_PATH.'icon_mt_flag_on.gif" title="This paragraph is to be translated into multiple languages" /></a>';
					} else {
						echo ' <a href="javascript:void(0);" onclick="AjaxPost(\'artworkID='.$artworkID.'&amp;p='.$row_para['ParaID'].'&amp;mt_flag=1\',\'\',\'modules/mod_art_amend.php\'); document.getElementById(\'mt_icon-'.$row_para['ParaID'].'\').src=\''.IMG_PATH.'icon_mt_flag_on.gif\';"><img id="mt_icon-'.$row_para['ParaID'].'" src="'.IMG_PATH.'mt_flag.jpg" title="Switch on multi-translation" /></a>';
					}
					// end flag
		...
		if($type == "PICT") {
			echo '<a href="javascript:void(0);" onclick="ResetDiv(\'window\');hidediv(\'helper\');SetClassName(\'guestTool\',\'pageToolOff\');SetClassName(\'amendTool\',\'pageToolOff\');SetClassName(\'commentTool\',\'pageToolOn\');display(\'pageColR\');ResetDiv(\'pageColR\');AjaxPost(\'artworkID='.$artworkID.'&page='.$page.'&edit_page='.$edit_page.'&boxID='.$boxID.'&reset=0\',\'pageColR\',\'modules/mod_page_comments.php\');"><img id="" src="'.IMG_PATH.'ico_comment.png" title="'.$lang->display('Add a Comment').'" /> '.$lang->display('Add a Comment').'</a>';
