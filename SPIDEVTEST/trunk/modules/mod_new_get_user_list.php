...
$source_lang_id = (!empty($_GET['source_lang_id'])) ? (int)$_GET['source_lang_id'] : 0;
$target_lang_id = (!empty($_GET['target_lang_id'])) ? (int)$_GET['target_lang_id'] : 0;
$subject_id = (!empty($_GET['subject_id'])) ? (int)$_GET['subject_id'] : 0;
$word_count = (!empty($_GET['word_count'])) ? (int)$_GET['word_count'] : 0;
			...
			<tr class="bgWhite" onmouseover="this.className='hover'" onmouseout="this.className='bgWhite'">
				<td>
					<input
						...
						onclick="if(confirm('<?php echo $lang->display('Are you sure you want to add this user?'); ?>')) { fadeOut('helper');ResetDiv('<?php echo $list; ?>');DoAjax('source_lang_id=<?php echo $source_lang_id; ?>&target_lang_id=<?php echo $target_lang_id; ?>&subject_id=<?php echo $subject_id; ?>&word_count=<?php echo $word_count; ?>&do=assign&ref=<?php echo GOOGLE_BOT_ID; ?>','<?php echo $list; ?>','modules/mod_new_<?php echo $target; ?>_list.php'); } else return false;"
					/>
				</td>
				<td>
			...
			<tr class="bgWhite" onmouseover="this.className='hover'" onmouseout="this.className='bgWhite'">
				<td>
					<input
						...
						onclick="if(confirm('<?php echo $lang->display('Are you sure you want to add this user?'); ?>')) { fadeOut('helper');ResetDiv('<?php echo $list; ?>');DoAjax('source_lang_id=<?php echo $source_lang_id; ?>&target_lang_id=<?php echo $target_lang_id; ?>&subject_id=<?php echo $subject_id; ?>&word_count=<?php echo $word_count; ?>&do=assign&ref=<?php echo $row_user['userID']; ?>','<?php echo $list; ?>','modules/mod_new_<?php echo $target; ?>_list.php'); } else return false;"
					/>
				</td>
				<td>
					<?php
