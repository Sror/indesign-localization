<?php
/**
 * PAGLACL - Generic Access Control List
 *
 * @package PAGLACL
 */
...
* PAGLACL main class
*
* Class acl should be used in applications where only querying the PAGLACL
...
class acl {
	/*
	--- PAGLACL Configuration path/file ---
	*/
	var $config_file = './acl.ini.php';
	...
	*/
	/** @var string Prefix for all the PAGLACL tables in the database */
	...
	/**
	 * Constructor
	 * @param array An arrn arry of options to oeverride the class defaults
	 */
	function acl($options = NULL) {

		$available_options = array('db','debug','items_per_page','max_select_box_items','max_search_return_items','db_table_prefix','db_type','db_host','db_user','db_password','db_name');
		...
		if ( file_exists($this->config_file) ) {
		        ...
		        if ( is_array($config) ) {
		                $acl_options = array_merge($config, $options);
		        }
