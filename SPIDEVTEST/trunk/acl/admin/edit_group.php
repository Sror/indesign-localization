...
switch ($_POST['action']) {
	case 'Delete':
		...
		if (count($_POST['delete_group']) > 0) {
			...
			foreach ($_POST['delete_group'] as $group_id) {
				$acl_api->debug_text('Deleting group_id: '. $group_id);
				
				$result = $acl_api->del_group($group_id, TRUE, $group_type);
				if ($result == FALSE) {
					$retry[] = $group_id;
				}
			}
			...
			if (count($retry) > 0) {
				foreach($retry as $group_id) {
					$acl_api->del_group($group_id, TRUE, $group_type);	
				}
			}
		...
		//Return page.
		$acl_api->return_page($return_page);
		break;
	case 'Submit':
		...
		if (empty($_POST['group_id'])) {
			$acl_api->debug_text('Insert');
			
			$insert_id = $acl_api->add_group($_POST['value'], $_POST['name'], $parent_id, $group_type);
		} else {
			$acl_api->debug_text('Update');
			
			$acl_api->edit_group($_POST['group_id'], $_POST['value'], $_POST['name'], $parent_id, $group_type);
		}
	...
	default:
		...
		$smarty->assign('options_groups', $acl_api->format_groups($acl_api->sort_groups($group_type)));
		break;
}
...
$smarty->display('paglacl/edit_group.tpl');
?>