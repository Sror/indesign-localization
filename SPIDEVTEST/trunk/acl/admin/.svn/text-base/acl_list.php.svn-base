...
switch ($_GET['action']) {
	case 'Delete':
		...
		//Return page.
		$acl_api->return_page($_GET['return_page']);
		break;
	case 'Submit':
	...
	default:
		...
		if (isset($_GET['action']) AND $_GET['action'] == 'Filter') {
			$acl_api->debug_text('Filtering...');
			
			$query = '
				SELECT		DISTINCT a.id
				FROM		'. $acl_api->_db_table_prefix .'acl a
				LEFT JOIN	'. $acl_api->_db_table_prefix .'aco_map ac ON ac.acl_id=a.id
				LEFT JOIN	'. $acl_api->_db_table_prefix .'aro_map ar ON ar.acl_id=a.id
				LEFT JOIN	'. $acl_api->_db_table_prefix .'axo_map ax ON ax.acl_id=a.id';
			...
			if ( isset($_GET['filter_aco']) AND $_GET['filter_aco'] != '') {
				$query .= '
				LEFT JOIN	'. $acl_api->_db_table_prefix .'aco c ON (c.section_value=ac.section_value AND c.value=ac.value)';
			...
			if ( isset($_GET['filter_aro']) AND $_GET['filter_aro'] != '') {
				$query .= '
				LEFT JOIN	'. $acl_api->_db_table_prefix .'aro r ON (r.section_value=ar.section_value AND r.value=ar.value)';
			...
			if ( isset($_GET['filter_aro_group']) AND $_GET['filter_aro_group'] != '') {
				$query .= '
				LEFT JOIN	'. $acl_api->_db_table_prefix .'aro_groups_map arg ON arg.acl_id=a.id
				LEFT JOIN	'. $acl_api->_db_table_prefix .'aro_groups rg ON rg.id=arg.group_id';
			...
			if ( isset($_GET['filter_axo']) AND $_GET['filter_axo'] != '') {
				$query .= '
				LEFT JOIN	'. $acl_api->_db_table_prefix .'axo x ON (x.section_value=ax.section_value AND x.value=ax.value)';
			...
			if ( isset($_GET['filter_axo_group']) AND $_GET['filter_axo_group'] != '') {
				$query .= '
				LEFT JOIN	'. $acl_api->_db_table_prefix .'axo_groups_map axg ON axg.acl_id=a.id
				LEFT JOIN	'. $acl_api->_db_table_prefix .'axo_groups xg ON xg.id=axg.group_id';
			...
			$query  = '
				SELECT a.id FROM ' . $acl_api->_db_table_prefix . 'acl a';
		}
		...
		if ( !($_GET['action'] == 'Filter' AND $acl_ids_sql == -1) ) {
			...
			$query = '
				...
				FROM	'. $acl_api->_db_table_prefix .'acl a
				INNER JOIN 	'. $acl_api->_db_table_prefix .'acl_sections x ON x.value=a.section_value
			...
			foreach ( array('aco', 'aro', 'axo') as $type ) {
				$query = '
					...
					FROM	'. $acl_api->_db_table_prefix . $type .'_map a
					INNER JOIN	'. $acl_api->_db_table_prefix . $type .' o ON (o.section_value=a.section_value AND o.value=a.value)
					INNER JOIN	'. $acl_api->_db_table_prefix . $type . '_sections s ON s.value=a.section_value
			...
			foreach ( array('aro', 'axo') as $type )
			{
				$query = '
					...
					FROM	'. $acl_api->_db_table_prefix . $type .'_groups_map a
					INNER JOIN	'. $acl_api->_db_table_prefix . $type .'_groups g ON g.id=a.group_id
					WHERE	a.acl_id IN ('. $acl_ids_sql . ')';
				$rs = $db->Execute($query);
		...
		foreach(array('aco','aro','axo','acl') as $type) {
			...
			$query = '
				SELECT value,name
				FROM '. $acl_api->_db_table_prefix .$type .'_sections
				WHERE hidden=0
...
$smarty->display('paglacl/acl_list.tpl');
?>