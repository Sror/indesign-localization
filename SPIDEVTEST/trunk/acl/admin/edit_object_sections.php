...
switch(strtolower(trim($object_type))) {
    ...
    case 'acl':
        $object_type = 'acl';
		$object_sections_table = $acl_api->_db_table_prefix . 'acl_sections';
        break;
    default:
...
switch ($_POST['action']) {
    case 'Delete':
        ...
        if (count($_POST['delete_sections']) > 0) {
            foreach($_POST['delete_sections'] as $id) {
                $acl_api->del_object_section($id, $object_type, TRUE);
            }
        }   
    ...
    case 'Submit':
        ...
        while (list(,$row) = @each($_POST['sections'])) {
            ...
            $acl_api->edit_object_section($id, $name, $value, $order,0,$object_type );
        }
        unset($id);
        unset($value);
        unset($order);
        ...
        while (list(,$row) = @each($_POST['new_sections'])) {
            ...
            if (!empty($value) AND !empty($order) AND !empty($name)) {
                ...
                $object_section_id = $acl_api->add_object_section($name, $value, $order, 0, $object_type);
                $acl_api->debug_text("Section ID: $object_section_id");
            }
        }
...
$smarty->display('paglacl/edit_object_sections.tpl');
?>
